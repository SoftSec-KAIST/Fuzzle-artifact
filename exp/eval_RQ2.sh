...
# algo
PARAM="Algorithm"
OUT_ALGO=$OUT_PATH/outputs/test_algo
python3 ./run_tools.py $CONFIG_DIR/test_algo.conf $OUT_ALGO
./save_results.sh $OUT_ALGO $OUT_ALGO/algo_results $PARAM $DURATION $MODE > $OUT_ALGO/algo_summary

# size
PARAM="Size"
OUT_SIZE=$OUT_PATH/outputs/test_size
python3 ./run_tools.py $CONFIG_DIR/test_size.conf $OUT_SIZE
./save_results.sh $OUT_SIZE $OUT_SIZE/size_results $PARAM $DURATION $MODE > $OUT_SIZE/size_summary

# cycle
PARAM="Cycle"
OUT_CYCLE=$OUT_PATH/outputs/test_cycle
python3 ./run_tools.py $CONFIG_DIR/test_cycle.conf $OUT_CYCLE
./save_results.sh $OUT_CYCLE $OUT_CYCLE/cycle_results $PARAM $DURATION $MODE > $OUT_CYCLE/cycle_summary

# smt
PARAM="Generator"
OUT_SMT=$OUT_PATH/outputs/test_smt
python3 ./run_tools.py $CONFIG_DIR/test_smt.conf $OUT_SMT
